...
In order to backtest a portfolio using [SPO](https://github.com/ForecastOS/forecastos/tree/v0.3.9/forecastos/portfolio/strategy/spo.py), we'll need:
...
For [TradingCost](https://github.com/ForecastOS/forecastos/tree/v0.3.9/forecastos/portfolio/cost_model/trading_cost.py), we'll need:
...
For [ShortHoldingCost](https://github.com/ForecastOS/forecastos/tree/v0.3.9/forecastos/portfolio/cost_model/short_holding_cost.py): we'll need:
...
from forecastos.portfolio.cost_model import *
from forecastos.portfolio.constraint_model import *

actual_returns = pd.read_parquet("https://forecastos.com/example_actual_returns.parquet")
forecast_returns = pd.read_parquet("https://forecastos.com/example_forecast_returns.parquet")

# For trading costs:
actual_prices = pd.read_parquet("https://forecastos.com/example_spo_actual_prices.parquet")
forecast_volume = pd.Series(
    pd.read_csv("https://forecastos.com/example_spo_forecast_volume.csv", index_col="asset")
    .squeeze(),
    name="forecast_volume"
)
forecast_std_dev = pd.Series(
    pd.read_csv("https://forecastos.com/example_spo_forecast_std_dev.csv", index_col="asset")
    .squeeze(),
...
```python
strategy = fos.portfolio.strategy.SPO(
...
portfolio = fos.portfolio.BacktestController(
    strategy=strategy,
...
If you have a charting library installed, like matplotlib, check out [BaseResult](https://github.com/ForecastOS/forecastos/tree/v0.3.9/forecastos/portfolio/result/base_result.py) for the many `backtest_result` metrics you can plot!

## Next: Reporting

Next, let's explore the backtest performance reporting available to you through `backtest_result` (an instance of BaseResult): [Analyzing Backtest Results](/guides/pm_reporting/analyzing_backtest_results).
